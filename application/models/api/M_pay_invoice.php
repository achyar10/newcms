...
class M_pay_invoice extends CI_Model {
	...
	function set_invoice($merchant_code){
		...
		if(count($inv) > 0){
			...
			$process = $this->db->update('pay_invoice', array('month' => $month, 'year' => $year,'invoice' => $invoice_number,'modified_date' => $now), array('merchant_code' => $merchant_code));
		}else{
			$invoice_number = 1;
		...
		if($process){
			return array(
				'result' => true,
				'inv' => 'PAY'.date('ym').'/'.$merchant_code."/".str_pad($invoice_number,4,"0",STR_PAD_LEFT)
			);
		}else{
