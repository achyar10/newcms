...
class Auth_manage extends CI_Controller {
    ...
    function login() {
        ...
        if ($_POST AND $this->form_validation->run() == TRUE) {
            ...
            if (count($user) > 0) {
                ...
                $this->session->set_userdata('uroleid', $user[0]['role_id']);
                $this->session->set_userdata('urolename', $user[0]['role_name']);
                $this->session->set_userdata('user_last_login', $user[0]['user_last_login']);
                $this->Users_model->add(array('user_id'=>$this->session->userdata('uid'), 'user_is_login'=>1));
                if ($location != '') {
